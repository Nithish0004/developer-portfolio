...
function buildFocusedDocument(variant: ElementVariant, size: number, particleAmount: number, mark: "openai" | "n" = "openai") {
...
(function () {
  ...
  let doc = applyDetailPatches(elementalMarksSource)
  ...
  if (mark === "n") {
    doc = doc
      .replace("logos.openai", "logos.n")
      .replace("logos.anthropic", "logos.n")
      .replace("logos.claude", "logos.n");
  }
...
export function ElementsBackground({
  ...
  const source = useMemo(
    () => buildFocusedDocument(safeVariant, size, particleAmount, mark),
    [mark, particleAmount, safeVariant, size],
  );
